...
object GoogleSignIn {
     ...
     * */
    fun initializeGoogleSignIn(context: Context, resultLauncher: ActivityResultLauncher<Intent>) =
        try {
    ...
    fun handleActivityResult(data: Intent?, socialLoginData: (SocialLoginData) -> Unit = {}) = try {
        val accountIntent = GoogleSignIn.getSignedInAccountFromIntent(data)
        val account = accountIntent.getResult(ApiException::class.java)
